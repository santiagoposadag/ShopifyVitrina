...
type InboundEvent struct {
	...
	Type      string     `json:"type"` // text | image | interactive | other
...
// downloadImage decrypts an inbound image straight to disk. Streaming to a file
...
func (h *Inbound) downloadImage(ctx context.Context, img whatsmeow.DownloadableMessage) (string, error) {
	f, err := h.stagingFile()
	if err != nil {
		return "", fmt.Errorf("create staging file: %w", err)
	}
	defer f.Close()
	if err := h.client.DownloadToFile(ctx, img, f); err != nil {
...
func (h *Inbound) Handle(rawEvt any) {
	...
	case msg.GetImageMessage() != nil:
		...
		path, err := h.downloadImage(ctx, img)
		if err != nil {
