...
| 15 | **Nothing records that an admin read a conversation.** The admin console authenticates every request and logs none of them: there is no audit trail saying which credential opened whose thread, or when. The credential is read-only, so the risk is not tampering — it is that a leaked or over-shared link is undetectable after the fact, and the thing it reads is every customer's phone number and messages (Ley 1581, see #7). A `request.log.info` naming the credential's `name` and the conversation key would close most of it, and deliberately must NOT include the token. Not done here because an audit trail with no retention policy is the same open question as #7, one table further down | `server/src/admin/console.ts` (the two data routes), `server/src/data/admin-roster.ts` | Medium |
