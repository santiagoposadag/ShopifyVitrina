...
| 18 | **A conversation can be left paused and forgotten, and nothing but a person notices.** Releasing is manual by design — a timer that resumed the bot would do it mid-exchange, with the human mid-sentence — so the accepted failure is the opposite one: an admin pauses a thread, gets distracted, and the customer is answered by nobody while their messages pile up recorded and unread. Mitigated only by visibility (the index badges paused rows and reports `pausedCount`); there is no alert, no age threshold, and nothing tells the admin who paused it. The likely fix is an owner notification once a handoff passes some age, which is a policy number nobody has chosen | `server/src/data/db.ts` (`conversation_handoff`, the no-auto-release note), `server/src/admin/console.ts` | **High** |
