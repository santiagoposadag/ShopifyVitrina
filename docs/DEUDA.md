...
| 7 | **`conversation_messages` is kept indefinitely — DECIDED, not pending.** The owner chose indefinite retention "por ahora" (2026-09-09) with the consequence stated: the table only grows, and `data/backup.ts` copies the whole SQLite file, so every conversation ever held rides in every backup copy. It stays listed because the decision is provisional and its cost compounds with volume, and because the stored words include the store's real CUSTOMERS — third parties to that decision, covered by Ley 1581. Revisit when the table is large enough to notice. The index is ordered so a sweep-by-age is a query rather than a migration | `server/src/data/db.ts` (the `conversation_messages` block states it), `server/src/data/repo.ts:471` sweeps only `inbox` | Medium (accepted) |
| 8 | **A purge cannot reach the words of a conversation whose session row is gone.** `purgeCustomerSessions` iterates `listSessions`, which filters `agent_session_id IS NOT NULL`, so a conversation whose session expired and was swept — or that an earlier purge cleared before message deletion existed — keeps its messages through every future run. Needs `listConversationKeysWithMessages` in the repo layer to union against; deliberately not worked around with raw SQL from `purge.ts`, which would bypass the seam every other caller of that table uses. Pinned by a test that fixes the honest behaviour rather than hiding it. **Raised to High by #7**: with retention indefinite, this purge is now the ONLY thing that ever deletes a conversation, so what it cannot reach is undeletable rather than merely late | `server/src/data/purge.ts` (comment after the customer loop), `server/src/data/repo.ts:104` | **High** |
| 9 | **System notices are sent but never recorded.** The rate-limit notice, the customer kill-switch notice and the terminal-failure apology go straight to `channel.sendText` instead of through the responder, so they never reach `conversation_messages`. They are messages the person genuinely received, so the record answers "what did we send this person" wrongly for exactly the cases someone would be investigating | `server/src/index.ts` (the notice sites), recorded path is `server/src/egress/responder.ts` | Medium |
...
| 12 | **The console link is a bearer credential that will live in a WhatsApp chat.** Whoever holds it can flip that phone's role, and owner role carries `set_price` and `delete_product` against a live store. Accepted deliberately: the holder is the store owner, who already has the Shopify admin, so the link grants nothing they do not have — the exposure is a leak, not the intended use. Mitigation is `rotate`, a short roster, and removing it when the test period ends | `server/src/data/test-console-credentials.ts` (`printLink` says so to the operator) | Medium |
