...
- **The storefront answers on two hosts, and the branded one is the exception, not the rule.** `STOREFRONT_BASE_URL` builds the branded links (`/propiedad`, `/preview`); `ANON_BASE_URL` builds `/ver/<token>` — a second domain on the *same* container, because the domain is the one piece of branding a de-branded page cannot hide from whoever reads the address bar. `(storefront)/layout.tsx` 404s the whole route group on the anonymous host, which is why the route group has to stay the exact boundary of what is branded: a new branded page added outside it is a page the guard does not cover. The 404 must not become a redirect — sending `/` to the branded domain announces the company to precisely the person the anonymous link hides it from (old customer links are grandfathered by a Cloudflare rule instead). `ANON_BASE_URL` empty falls back to the branded host and makes every check inert; `web/lib/hosts.ts` and `server/src/config.ts` must keep that fallback identical or the server mints links the web app refuses to serve.
